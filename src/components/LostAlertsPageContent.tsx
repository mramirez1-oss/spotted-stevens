...
export function LostAlertsPageContent() {
  const [alerts, setAlerts] = useState<LostAlert[] | null>(null);
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setFetchError(data.error ?? "Could not load lost alerts.");
          setUsingMock(false);
          return;
        }
        setAlerts(data.alerts ?? []);
        ...
        if (!cancelled) {
          setAlerts([]);
          setFetchError("Could not load lost alerts.");
        }
      }
    }
    ...
    <div className="min-h-screen">
      <div className="border-b border-spot-sky/40 bg-gradient-to-br from-white via-spot-cream to-spot-sky/20">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          {alerts !== null && !usingMock && (
            <p className="mt-3 text-sm text-spot-blue/80">
              {fetchError
                ? "Could not refresh the feed."
                : `${count} alert${count === 1 ? "" : "s"} · newest first from Supabase`}
            </p>
          )}
          {usingMock && (
            <p className="mt-4 rounded-xl border border-spot-sky/60 bg-white/80 px-3 py-2 text-sm text-spot-navy shadow-sm">
              ...
              in your environment to load and post real alerts.
            </p>
          )}
          {alerts !== null && fetchError && !usingMock && (
            <div
      ...
      <main className="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {alerts === null ? (
          ...
          <p className="rounded-2xl border border-dashed border-spot-sky/60 bg-white/60 px-6 py-12 text-center text-spot-blue">
            ...
            {usingMock ? " once Supabase is set up" : ""}.
          </p>
        ) : (
          <ol className="flex flex-col gap-4">
            {list?.map((alert, index) => (
              <li key={alert?.id ?? `alert-${index}`}>
